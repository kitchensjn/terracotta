...
pruning<-function(q,tree,x,model=NULL,...){
  ...
  for(i in 1:length(nn)){
      ee<-which(pw$edge[,1]==nn[i]) #children
...
write.csv(eel.data, file="/Users/jameskitchens/Documents/GitHub/terracotta/devlog/20260309/phytools_comparison/data/feeding_mode.csv")
write.tree(eel.tree, file="/Users/jameskitchens/Documents/GitHub/terracotta/devlog/20260309/phytools_comparison/data/tutorial_tree.newick")
...
#sunfish.data$feeding.mode <- c("bite", "pisc", "non", "bite", "pisc", "non", "non")

#feeding_mode <- factor(sunfish.data$feeding.mode)

fmode<-setNames(sunfish.data$feeding.mode, #feeding_mode,
                rownames(sunfish.data))

## set our model design (this is the ARD model)
#model<-matrix(c(0,1,2,3,0,4,5,6,0),3,3,byrow=TRUE)
model<-matrix(c(0,1,2,0),2,2,byrow=TRUE)
...
plotTree(sunfish.tree,lwd=1,ftype="i",direction="upwards",offset=1,
         fsize=0.7)
nodelabels()
...
write.csv(sunfish.data, file="/Users/jameskitchens/Documents/GitHub/terracotta/devlog/20260309/phytools_comparison/data/feeding_mode.csv")
write.tree(sunfish.tree, file="/Users/jameskitchens/Documents/GitHub/terracotta/devlog/20260309/phytools_comparison/data/tutorial_tree.newick")
