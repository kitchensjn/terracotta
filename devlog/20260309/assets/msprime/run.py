...
class WorldMap:
    ...
    def build_transition_matrices(self, parameters):
        ...
        for e in range(len(self.epochs)):
            all_suitabilities = self.demes[f"suitability_{e}"]**abs(a)
            scaled_suitabilities = all_suitabilities/sum(all_suitabilities)
            for _,connection in self.connections.iterrows():
                ...
                if a > 0:
                    suit = scaled_suitabilities[i_1]
                else:
                    suit = scaled_suitabilities[i_0]
